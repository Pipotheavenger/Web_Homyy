    ...
    <main className="relative min-h-screen flex flex-col items-center justify-center px-2 sm:px-4 bg-lavender overflow-hidden">
      ...
      <div className="flex flex-col items-center gap-3 mb-2 sm:mb-4 md:mb-6 animate-fade-in-up">
        ...
        <Image
          ...
          />
          <div className="absolute inset-0 bg-gradient-to-br from-purple-400 to-purple-600 rounded-full opacity-0 group-hover:opacity-20 transition-opacity duration-300" />
        </div>
      ...
      <section className="w-full max-w-xs sm:max-w-sm md:max-w-md bg-white/90 backdrop-blur-xl rounded-2xl border border-white/20 shadow-xl p-3 sm:p-6 md:p-8 space-y-4 sm:space-y-6 pb-0 animate-fade-in-up animation-delay-200">
        <form onSubmit={handleSubmit} className="space-y-3 sm:space-y-4 md:space-y-6 text-sm sm:text-base">
          ...
          <div className="space-y-1">
            <label className="block relative group">
              ...
              <input
                ...
                onBlur={() => setFocusedField(null)}
                className={`w-full pl-10 pr-8 py-3 rounded-xl border-2 transition-all duration-300 bg-white/70 backdrop-blur-sm placeholder-gray-400 font-medium text-sm sm:text-base ${
                  errors.password
                    ? 'border-red-300 focus:border-red-500 focus:ring-red-500/20'
              ...
              <button
                ...
                className="absolute right-2 top-1/2 -translate-y-1/2 text-gray-400 hover:text-purple-600 transition-all duration-300 hover:scale-110 focus:outline-none focus:scale-110"
              >
                <div className="relative">
                  <div className={`transition-all duration-300 ${showPwd ? 'opacity-0 rotate-180' : 'opacity-100 rotate-0'}`}>
                  ...
                  <div className={`absolute inset-0 transition-all duration-300 ${showPwd ? 'opacity-100 rotate-0' : 'opacity-0 -rotate-180'}`}>
                    <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" strokeWidth={1.8}>
          ...
          <div className="flex justify-between items-center text-xs sm:text-sm text-gray-600">
            <label className="flex items-center gap-1 cursor-pointer group">
              <input 
                ...
                className="w-3 h-3 sm:w-4 sm:h-4 text-purple-600 border-2 border-gray-300 rounded focus:ring-purple-500 focus:ring-2 transition-all duration-200 hover:border-purple-400"
              />
              <span className="font-medium group-hover:text-gray-800 transition-colors">Recuérdame</span>
            </label>
            <a href="#" className="text-purple-600 hover:text-purple-700 font-medium hover:underline transition-all duration-200 text-right leading-tight">
              ¿No puedes entrar?<br />
              <span className="text-purple-500">Recupera tu contraseña</span>
        </a>
          </div>
          ...
          <p className="text-center text-xs sm:text-sm text-gray-600 font-medium mb-0 pb-0">
            ¿Primera vez aquí?{' '}
            <a href="#" className="text-purple-600 font-bold hover:text-purple-700 hover:underline transition-all duration-200">
              ¡Regístrate gratis!
            </a>
          </p>
        </form>
